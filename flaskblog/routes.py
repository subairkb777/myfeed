...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Resuest', sender='admin@lol.com', recipients=[user.email])
    msg.body = f'''To rest Your password visit following
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash("Email send. Check mail", 'info')
        return redirect(url_for('login'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your password changed. Login Now!', 'success')
        return redirect(url_for('login'))
